...
int main(int argc, char *argv[]){
  ...
  // ciclo che stampa il messaggio iniziale senza andare in conflitto con la stampa del messaggio
  for(nwrite = 0; nwrite < 56; nwrite++)
  {
	 if(read(socketfd,&chMaiu,1) > 0){
		 printf("%c",chMaiu);
		 buffer2[nwrite] = chMaiu;
	 }
  }
