...
- Un altro attacco appartenente alla famiglia DoS è il **Distributed Denial of Service** (DDoS).
  ...
  Il DDoS consiste nell'utilizzo di diversi dispositivi, e non solamente di uno come nel DoS, che prendono di mira una singola risorsa. 
...
[DoS e DDoS](https://it.wikipedia.org/wiki/Denial_of_service)[DoS](https://academy.binance.com/it/security/what-is-a-dos-attack)
...
- La seconda tipologia di errore che si può incontrare è la gestione non corretta del **tipo**.
  ...
  ```sql
  "SELECT * FROM utenti WHERE id =" + inpututenti + ";"
  ```

  Inserendo in input 

  **1;DROP TABLE utenti** la query si trasformerà quindi in 

  ```sql
  "SELECT * FROM utenti WHERE id =1; DROP TABLE utenti;"
  ```

  andando ad eliminare la tabella utenti.
...
- Un altro strato di protezione da attacchi SQL injection che si può introdurre riguarda i **permessi**.
  ...
  ```sql
  deny select on utente.id_utente to inputricerca;
  ```

  In questo modo il soggetto attaccante non riuscirà ad ottenere i dati a cui stava mirando.
